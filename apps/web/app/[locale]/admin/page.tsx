    ...
    <div className="relative min-h-screen overflow-x-hidden">
      ...
      <header className="sticky top-0 z-50 border-b border-white/[0.08] bg-white/[0.04] backdrop-blur-xl">
        <div className="mx-auto flex max-w-5xl items-center justify-between px-6 py-4">
          ...
          <span className="rounded-full border border-white/10 bg-white/[0.06] px-2.5 py-1 text-[11px] font-medium uppercase tracking-wider text-white/40">
            Admin
          </span>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-5xl px-6 py-10">
        ...
        <div className="mb-8">
          ...
          <p className="mt-1 text-[14px] text-white/40">{t.subtitle}</p>
        </div>

        {/* Add user form */}
        <form
          action={addUser}
          className="mb-8 flex flex-col gap-3 rounded-2xl border border-white/[0.08] bg-white/[0.04] p-5 backdrop-blur-md sm:flex-row sm:items-end"
        >
          <div className="flex-1">
            ...
            <input
              ...
              className="w-full rounded-xl border border-white/10 bg-white/[0.06] px-4 py-2.5 text-[14px] text-white placeholder-white/20 outline-none backdrop-blur-md transition-all focus:border-white/25 focus:bg-white/[0.09]"
            />
          </div>
          <div>
            <label className="mb-1.5 block text-[12px] font-medium text-white/40">
            ...
            <select
              name="role"
              className="rounded-xl border border-white/10 bg-[oklch(0.12_0.02_260)] px-4 py-2.5 text-[14px] text-white/80 outline-none backdrop-blur-md transition-all focus:border-white/25"
            >
            ...
            <thead>
              <tr className="border-b border-white/[0.06] bg-white/[0.03]">
                {[t.nameCol, t.emailCol, t.roleCol, t.statusCol, t.actionsCol].map((h) => (
                  <th key={h} className="px-5 py-3 text-start text-[11px] font-medium uppercase tracking-wider text-white/30">
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {users.map((user, i) => {
                ...
                return (
                  <tr
                    key={user.id}
                    className={`border-b border-white/[0.04] transition-colors hover:bg-white/[0.03] ${i === users.length - 1 ? "border-none" : ""}`}
                  >
                    {/* Name */}
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        {user.image ? (
                          ...
                          <img src={user.image} alt="" width={32} height={32} className="rounded-full ring-1 ring-white/10" />
                        ) : (
                          <div className="flex h-8 w-8 items-center justify-center rounded-full bg-white/10 text-[12px] font-semibold uppercase text-white/60">
                        ...
                        <div>
                          <p className="text-[13px] font-medium text-white/80">
                            {user.name ?? "—"}
                            {isSelf && <span className="ms-1.5 text-[10px] text-white/30">({t.you})</span>}
                          </p>
                        </div>
                      </div>
                    </td>
                    ...
                    {/* Email */}
                    <td className="px-5 py-4 text-[13px] text-white/50">{user.email}</td>

                    {/* Role */}
                    <td className="px-5 py-4">
                      <span className={`rounded-full px-2.5 py-0.5 text-[11px] font-medium ${
                        user.role === "admin"
                          ? "border border-[oklch(0.5706_0.2236_258.71/0.4)] bg-[oklch(0.5706_0.2236_258.71/0.12)] text-[oklch(0.75_0.15_258.71)]"
                    ...
                    <td className="px-5 py-4">
                      <span className={`rounded-full px-2.5 py-0.5 text-[11px] font-medium ${
                        user.active
                          ? "border border-[oklch(0.72_0.17_142/0.4)] bg-[oklch(0.72_0.17_142/0.12)] text-[oklch(0.80_0.14_142)]"
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {!isSelf && (
                          <form action={toggleActive}>
                            ...
                            <button
                              type="submit"
                              className="rounded-lg border border-white/10 bg-white/[0.05] px-3 py-1.5 text-[12px] text-white/50 transition-all hover:border-white/20 hover:bg-white/[0.09] hover:text-white/80"
                            >
