...
export default function InsightsClient({ data, locale, t, month }: Props) {
  ...
  return (
    <div>
      ...
      <div className="mb-8">
        <h1 className="text-[28px] font-semibold tracking-tight text-white mb-6">{t.title}</h1>
        <div className="flex items-center justify-center gap-4">
          <button
            onClick={() => navigateMonth(-1)}
            className="rounded-xl border border-white/10 bg-white/[0.06] p-2 text-white/50 transition-all hover:bg-white/[0.10] hover:text-white/80"
          >
            <ChevronLeftIcon />
          </button>
          ...
          <button
            ...
            className={cn(
              "rounded-xl border border-white/10 bg-white/[0.06] p-2 transition-all",
              isCurrentMonth ? "cursor-not-allowed text-white/25" : "text-white/50 hover:bg-white/[0.10] hover:text-white/80",
            )}
          >
            <ChevronRightIcon />
          </button>
      ...
      <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="relative overflow-hidden rounded-3xl border border-white/[0.10] bg-white/[0.05] p-6 backdrop-blur-xl">
        ...
        <div className="relative overflow-hidden rounded-3xl border border-white/[0.10] bg-white/[0.05] p-6 backdrop-blur-xl">
          ...
          <div className="relative">
            <p className="text-[12px] font-medium uppercase tracking-wider text-white/40">{t.net}</p>
            ...
            <p className="mt-2 text-[12px] text-white/35">{t.vsLastMonth}</p>
          </div>
        </div>
      </div>
      ...
      {data.categoryBreakdown.length > 0 ? (
        <div className="mb-8 rounded-3xl border border-white/[0.08] bg-white/[0.04] p-6 backdrop-blur-xl">
          <h2 className="mb-4 text-[16px] font-semibold text-white">{t.breakdownTitle}</h2>
              ...
              <div key={row.category_id ?? "uncategorized"} className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[13px] text-white/80">{getCatName(row.category_name_en, row.category_name_he)}</span>
                  </div>
                  <span className="text-[13px] font-medium text-white">{row.pct.toFixed(0)}%</span>
                </div>
                ...
                <div className="text-[12px] text-white/40">{fmtCurrency(row.total)}</div>
              </div>
            ))}
          </div>
        </div>
      ) : (
        <div className="mb-8 rounded-3xl border border-white/[0.08] bg-white/[0.04] p-6 text-center backdrop-blur-xl">
          <p className="text-[14px] text-white/40">{t.noExpenses}</p>
        </div>
      )}
      ...
      {data.topExpenses.length > 0 && (
        <div className="mb-8 rounded-3xl border border-white/[0.08] bg-white/[0.04] p-6 backdrop-blur-xl">
          <h2 className="mb-4 text-[16px] font-semibold text-white">{t.topExpensesTitle}</h2>
          <div className="space-y-3">
            {data.topExpenses.map((tx, i) => (
              <div key={tx.id} className="flex items-center justify-between gap-3 rounded-xl border border-white/[0.06] bg-white/[0.02] p-3">
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  <span className="text-[12px] font-medium text-white/50">#{i + 1}</span>
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-[13px] text-white/80">{tx.description}</p>
                    <p className="text-[11px] text-white/35">{fmtDate(tx.date)}</p>
                  </div>
                </div>
                <div className="flex flex-col items-end gap-1 shrink-0">
                  <p className="text-[13px] font-medium text-white">{fmtCurrency(tx.amount)}</p>
                  {tx.category_emoji && (
      ...
      <div className="mb-8 rounded-3xl border border-white/[0.08] bg-white/[0.04] p-6 backdrop-blur-xl">
        <h2 className="mb-4 text-[16px] font-semibold text-white">{t.paceTitle}</h2>
        <div className="space-y-4">
          <div>
            <div className="mb-2 flex items-center justify-between">
              <span className="text-[13px] text-white/60">{t.activeDays}</span>
          ...
          <div className="flex items-center justify-between rounded-xl border border-white/[0.06] bg-white/[0.02] p-3">
            <span className="text-[13px] text-white/60">{t.avgPerDay}</span>
            <span className="text-[16px] font-semibold text-white">{fmtCurrency(data.spendingPace.avg_per_day)}</span>
          </div>
        </div>
      </div>
      ...
      {data.categoryTrends.length > 0 && (
        <div className="rounded-3xl border border-white/[0.08] bg-white/[0.04] p-6 backdrop-blur-xl">
          <h2 className="mb-4 text-[16px] font-semibold text-white">{t.trendsTitle}</h2>
          <div className="space-y-3">
            {data.categoryTrends.map((row) => {
              ...
              return (
                <div key={row.category_id ?? "uncategorized"} className="flex items-center justify-between gap-3 rounded-xl border border-white/[0.06] bg-white/[0.02] p-3">
                  <div className="flex items-center gap-2 min-w-0 flex-1">
                    <span className="text-[16px]">{row.category_emoji || "🏷️"}</span>
                    <span className="truncate text-[13px] text-white/80">{getCatName(row.category_name_en, row.category_name_he)}</span>
                  </div>
                  <div className="flex items-center gap-3 shrink-0">
                    <div className="text-end">
                      <p className="text-[13px] font-medium text-white">{fmtCurrency(current)}</p>
                      <p className={cn("text-[11px]", delta.sign === "positive" ? "text-[oklch(0.80_0.14_142)]" : delta.sign === "negative" ? "text-[oklch(0.78_0.16_27)]" : "text-white/40")}>
                        {delta.label}
                      </p>
                    </div>
                  </div>
