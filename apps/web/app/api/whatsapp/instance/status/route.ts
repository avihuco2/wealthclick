...
export async function GET() {
  ...
  `;
  if (!cfg) return NextResponse.json({ instance: { state: "unconfigured" } });

  const apiKey = decryptApiKey(cfg.api_key_enc, cfg.api_key_iv, cfg.api_key_tag);
  try {
    const result = await getInstanceStatus({ url: cfg.evolution_url, apiKey, instance: cfg.instance_name });
    return NextResponse.json(result);
  } catch {
    return NextResponse.json({ instance: { state: "error" } });
  }
}
